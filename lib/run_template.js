var fs              = require('fs');
var gitClone        = require('git-clone');
var mkdirp          = require('mkdirp');
var rimraf          = require('rimraf');
var join            = require('path').join;
var tmp             = require('tmp');

var getTemplatePath = require('./template_path');
var parser          = require('./parser');
var Context         = require('./Context');
var evaluate        = require('./evaluate');

module.exports = runTemplate;
function runTemplate(template, targetPath, subdir, cb) {

    var templateRoot    = null;
    var templatePath    = null;
    var plan            = null;

    if (fs.existsSync(targetPath)) {
        return cb(new Error("exists: " + targetPath));
    }

    getTemplatePath(template, '.', function(err, tp) {
        if (err) return cb(err);
        if (tp.type === 'local') {
            templateRoot = tp.path;
            _loadPlan();
        } else {
            _cloneRepo(tp);
        }
    });

    function _cloneRepo(repo) {
        tmp.dir({unsafeCleanup: true}, function(err, tmpDir) {
            if (err) return cb(err);
            templateRoot = join(tmpDir, 'clone');
            gitClone(repo.url, templateRoot, {checkout: repo.checkout}, function(err) {
                if (err) return cb(err);
                rimraf(join(templateRoot, '.git'), function(err) {
                    if (err) return cb(err);
                    _loadPlan();
                });
            });
        });
    }

    function _loadPlan() {

        templatePath = join(templateRoot, subdir);

        var planSource;
        try {
            planSource = fs.readFileSync(join(templatePath, 'plan.tpl'), 'utf8');
        } catch (err) {
            planSource = 'tree';
        }

        try {
            plan = parser.parse(planSource, {startRule: 'Script'});
        } catch (err) {
            // TODO: need a way to return a descriptive error message
            // with optional detailed error information.
            return cb(err);
        }

        _createTargetDirectory();

    }

    function _createTargetDirectory() {

        try {
            mkdirp.sync(targetPath);
        } catch (err) {
            // TODO: again, need a nice way of reporting this
            return cb(err);
        }

        _run();

    }

    function _run() {

        var previousDirectory = process.cwd();
        try {
            process.chdir(targetPath);
        } catch (e) {
            return cb(err);
        }

        var ctx = new Context(templatePath, targetPath);

        evaluate.run(plan, ctx, function(err) {
            try {
                process.chdir(previousDirectory);
            } catch (e) {
                // TODO: any point in worrying about this?
            }
            if (err) return cb(err);
            cb();
        });

    }

}