...
function evalSequence(ctx, env, list, cb) {
    (function next(ix) {
        if (ix === list.length) return cb(null);
        evalDirective(ctx, env, list[ix], function(err) {
...
function evalIfStatement(ctx, env, node, cb) {
    try {
        var condition = expressions.evaluate(env, node.cond);
        var truthy = expressions.isTruthy(condition);
        evalSequence(ctx, env, node[truthy ? body : alt], cb);
    } catch (err) {
        cb(err);
    }
}
